...
async function sendDeliverable(botToken: string, chatId: number, deliverable: Deliverable) {
  console.log(`[PaymentSync] Enviando entregavel: ${deliverable.name} (${deliverable.type})`)
...
// ---------------------------------------------------------------------------
// Worker principal - roda em loop infinito
// ---------------------------------------------------------------------------

async function syncPayments() {
  if (!SUPABASE_SERVICE_KEY) {
    console.log("[PaymentSync] SUPABASE_SERVICE_ROLE_KEY nao configurada, pulando...")
    return
  }
  ...
  try {
    ...
    console.log(`[PaymentSync] Verificando ${payments.length} pagamentos pendentes`)
    ...
    for (const payment of payments) {
      try {
        ...
        if (mpStatus !== payment.status) {
          ...
          console.log(`[PaymentSync] Payment ${payment.external_payment_id}: ${payment.status} -> ${mpStatus}`)
        }
        ...
        if (mpStatus === "approved") {
          ...
          console.log(`[PaymentSync] Entrega enviada para ${chatId}`)
        }
      } catch (err) {
        console.error(`[PaymentSync] Erro ao processar payment ${payment.id}:`, err)
      }
    }
    
  } catch (err) {
    console.error("[PaymentSync] Erro geral:", err)
  }
}

// ---------------------------------------------------------------------------
// Funcao que inicia o worker
// ---------------------------------------------------------------------------

export function startPaymentSyncWorker() {
  if (isRunning) {
    console.log("[PaymentSync] Worker ja esta rodando")
    return
  }
  
  isRunning = true
  console.log(`[PaymentSync] Worker iniciado - verificando a cada ${SYNC_INTERVAL / 1000}s`)
